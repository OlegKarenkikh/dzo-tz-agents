...
| [📋 Алгоритм Агента ДЗО](agent1_dzo_inspector/ALGORITHM.md) | все | Бизнес-правила работы Инспектора заявок ДЗО на русском языке |
| [📋 Алгоритм Агента ТЗ](agent2_tz_inspector/ALGORITHM.md) | все | Бизнес-правила работы Инспектора технических заданий на русском языке |
...
# ⚠️ ВАЖНО: НЕ заполняйте OPENAI_API_KEY
# ✅ Оставьте пустым — автоматически используется GITHUB_TOKEN

pip install -e ".[ui,dev]"

make api-ui   # FastAPI + Streamlit
# → автоматически использует GitHub Models (gpt-4o, Llama и др.)
```

> **Примечание:** Если вам нужен конкретный LLM backend, установите `LLM_BACKEND=github_models` (или `LLM_BACKEND=openai` с `OPENAI_API_KEY`) в `.env` явно.
