...
graph TD
    ...
    subgraph "REST API (api/app.py)"
        ...
        JOBS[💾 Jobs Store<br/>in-memory dict]
    end
...
2. API → создаёт job в памяти (UUID), возвращает job_id
...
- Хранение заданий в памяти (`dict` с UUID)
...
Не работает напрямую с агентами или IMAP/SMTP.
...
| `tools.py` | Инструменты агента (LangChain tools): отчёты, формы, письма |
...
| `runner.py` | Оркестратор: IMAP → текст → агент → SMTP |
| `tools.py` | Инструменты: JSON-отчёт, исправленное ТЗ, письмо ДЗО |
