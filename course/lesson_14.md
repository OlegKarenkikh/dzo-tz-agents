...
> ⏱️ **25 минут** | 🟠 Средний
> ⬅️ [Урок 13 — Защита LLM](lesson_13.md)

> 🎯 **Зачем этот урок?** Агент Тендер — второй по важности в проекте. Ты поймёшь как он взаимодействует с агентом ДЗО и что формирует на выходе.
...
> 💡 **Аналогия:** Агент Тендер — как тендерный менеджер: получает задание от руководства (ТЗ), изучает документацию (ДЗО) и готовит пакет заявки (Collector).
...
## ⚠️ Подводные камни для новичка

> ⚠️ **Не путай агентов местами:** Тендер НЕ может вызвать ДЗО — только ДЗО вызывает Тендер.
> Если попытаться, агент упадёт с ошибкой `peer_agent_unavailable`.

> ⚠️ **`job_id` — не случайная строка.** Это UUID, который создаётся агентом ДЗО.
> Тендер получает его как параметр — нельзя придумать его самому.

> ⚠️ **44-ФЗ и 223-ФЗ требуют разных сроков.** Если перепутать тип закупки, SLA-проверка вернёт ошибку `SLA_violation`.
...
1. Сколько агентов участвуют в обработке тендерного задания?
2. Кто вызывает агента Тендер — ТЗ или ДЗО?
3. Что делает агент Тендер с результатами от агента ДЗО?
4. Найди в коде метод `invoke_peer_agent()` — в каком файле он?
