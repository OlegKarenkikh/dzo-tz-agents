...
> ⏱️ **25 минут** | 🟡 Начинающий
> ⬅️ [Урок 4 — Токен и .env](lesson_04_token.md)
...
## ⚠️ Типичные ошибки при работе с LLM

| Ошибка | Симптом | Решение |
|---|---|---|
| `temperature=1.0` для структурированных данных | Агент выдаёт разные форматы каждый раз | Использовать `temperature=0` для детерминизма |
| Слишком большой контекст | `context_length_exceeded` ошибка | Обрезать входные данные перед отправкой |
| Не обрабатывать `RateLimitError` | Агент падает при нагрузке | Добавить retry с экспоненциальным back-off |
| Доверять ответу LLM без валидации | Галлюцинации попадают в продакшн | Парсить через Pydantic + проверять бизнес-правила |


## ✅ Проверь себя

1. Что такое токен в контексте LLM — единица слова или буква?
2. Как температура `0.0` и `1.0` влияют на ответ модели?
3. Почему нельзя передавать неограниченный контекст?
4. Что такое `system prompt` и чем он отличается от `user message`?
5. Найди в коде проекта где устанавливается `temperature` для агента ДЗО.
