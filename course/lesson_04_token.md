...
> ⏱️ **15 минут** | 🟡 Начинающий
> ⬅️ [Урок 3 — curl и HTTP](lesson_03_curl.md)

> 🎯 **Зачем этот урок?** Без правильного хранения токенов API-ключ попадёт в публичный git и тебя взломают. Ты узнаешь единственно правильный способ — через `.env` файл.
...
## ⚠️ Типичные ошибки с токенами — цена ошибки высока

| Ошибка | Последствие | Решение |
|---|---|---|
| `API_KEY=sk-proj-...` прямо в коде | Токен попадает в git history — нужно перегенерировать | Использовать только `.env` + `os.getenv()` |
| Добавил `.env` в git | Ключ публичен — кто угодно может использовать | Немедленно: `git rm --cached .env`, добавить в `.gitignore` |
| Скопировал токен в Slack/Telegram | История чатов хранится на серверах | Никогда не делиться токенами в мессенджерах |
| Один токен для всего | При компрометации падает вся система | Разные токены для dev/staging/prod |


## ✅ Проверь себя

1. Что такое токен API и почему его нельзя хранить прямо в коде?
2. Где хранится токен в проекте `dzo-tz-agents`?
3. Как прочитать переменную окружения из Python-кода?
4. Что будет, если файл `.env` попадёт в `git push`?
5. Открой файл `.env.example` в репозитории — какие токены там перечислены?
