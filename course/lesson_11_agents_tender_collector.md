...
> ⏱️ **25 минут** | 🟠 Средний
> ⬅️ [Урок 10 — Агент ТЗ изнутри](lesson_10_agent_tz.md)
...
## ⚠️ Типичные ошибки при работе с Агентами Тендер и Collector

| Ошибка | Симптом | Решение |
|---|---|---|
| Путать Прямые и Косвенные требования | Пропуск документов → отклонение заявки | Всегда проверять оба типа в ответе агента |
| Вызывать Collector напрямую из Тендер | Нарушение архитектуры — жёсткая зависимость | Только через Агент ДЗО с `invoke_peer_agent` |
| Не обрабатывать `inn_not_found` | Система продолжает без проверки ИНН | Останавливать процесс и запрашивать корректный ИНН |
| Игнорировать `reqs_count=0` в ответе Тендер | Пустой результат принимается как успех | Проверять что `reqs_count > 0` перед продолжением |
