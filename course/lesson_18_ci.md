...
> 💡 **Аналогия:** CI — как автоматический контролёр качества на заводе: каждая деталь (коммит) проходит проверку перед тем как попасть в готовый продукт. Если деталь бракованная — конвейер останавливается.
...
| Понятие | Смысл |
|---|---|
| CI | Автоматическая проверка при каждом push |
| GitHub Actions | Инструмент CI — запускает `.github/workflows/*.yml` |
| workflow | Файл с описанием шагов проверки (YAML) |
| Зелёный pipeline | Все проверки прошли — можно мержить |
| Красный pipeline | Что-то упало — сначала починить |
| Secrets | Секреты (API-ключи) хранятся в GitHub Settings, не в коде |
