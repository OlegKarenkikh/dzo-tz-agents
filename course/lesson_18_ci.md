...
> ⏱️ **25 минут** | 🔴 Продвинутый
> ⬅️ [Урок 17 — Тестирование](lesson_17_testing.md)
...
> 💡 **Аналогия:** CI — как автоматический контролёр качества: каждый коммит проходит проверку перед попаданием в продакшн. Красный статус = стоп, сначала починить.
...
| Понятие | Смысл |
|---|---|
| CI | Авто-проверка при каждом push |
| GitHub Actions | Запускает `.github/workflows/*.yml` |
| `ci.yml` | Lint + unit + integration тесты |
| `deploy.yml` | Деплой после зелёного CI |
| Зелёный статус | Можно мержить |
| Красный статус | Сначала починить |

---

## ⚠️ Типичные ошибки при работе с CI/CD

| Ошибка | Симптом | Решение |
|---|---|---|
| Секреты в коде вместо GitHub Secrets | API-ключ виден в публичном репозитории | Хранить только в `Settings → Secrets` |
| Запускать E2E в CI без мока | Тесты тратят деньги на API при каждом push | Использовать заглушку `sk-unit-test` в CI |
| Мержить при красном pipeline | Баг попадает в main | Настроить branch protection rule |
| Не кешировать `pip install` в CI | Каждый прогон устанавливает пакеты заново (~2мин) | Добавить `cache: pip` в workflow |
