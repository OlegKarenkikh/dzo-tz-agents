...
> ⬅️ [Урок 5 — LLM мозг агента](lesson_05_llm.md)

> 🎯 **Зачем этот урок?** Инструменты — это то, чем агент отличается от простого чат-бота. После урока ты поймёшь как LLM «решает» вызвать функцию Python.
...
## ⚠️ Типичные ошибки при написании инструментов

| Ошибка | Почему плохо | Решение |
|---|---|---|
| Инструмент делает слишком много | LLM путается что и когда вызывать | Один инструмент — одна задача |
| Нет обработки ошибок внутри tool | Исключение прерывает всю цепочку агента | Оборачивать в `try/except`, возвращать статус |
| Возвращать сырой объект вместо строки | LLM не может прочитать Python-объект | Конвертировать результат в `str` или JSON |
| Называть инструмент неинформативно | `tool_1()`, `process()` — LLM выбирает случайно | Имя должно описывать действие: `validate_inn()` |
