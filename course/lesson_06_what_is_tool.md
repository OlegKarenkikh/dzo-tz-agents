...
1. Что такое `@tool` декоратор — для чего он нужен?
2. Почему docstring у `@tool` функции критически важен?
3. Как LLM решает какой инструмент вызвать?
4. Что вернёт инструмент если ИНН не найден в РНП?
5. Напиши заготовку `@tool` функции `get_company_name(inn: str) -> dict`.
