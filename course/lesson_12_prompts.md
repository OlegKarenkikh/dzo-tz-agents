...
> ⏱️ **40 минут** | 🟠 Средний
> ⬅️ [Урок 11 — Тендер и Collector](lesson_11_agents_tender_collector.md)

> 🎯 **Зачем этот урок?** Промпт — это код для LLM. Плохой промпт = непредсказуемый агент. Ты узнаешь как писать промпты которые работают стабильно.
...
> 💡 **Аналогия:** Промпт — как должностная инструкция для сотрудника. Чем точнее написано что делать и чего не делать — тем предсказуемее результат.
...
> FastAPI (веб-фреймворк проекта) ожидает JSON в определённом формате. Если агент вернул текст вместо JSON:
...
> 💡 **Конкретный синтаксис few-shot в промпте:**
> ```markdown
> ## Примеры правильных решений
>
> **Заявка 1 (полная):**
> Документ: "ООО Ромашка, ИНН: 1234567890, место поставки: г. Москва"
> Решение: `{"decision": "Заявка полная", "missing_fields": []}`
>
> **Заявка 2 (требует доработка):**
> Документ: "ООО Ромашка, закупка ноутбуков"
> Решение: `{"decision": "Требуется доработка", "missing_fields": ["ИНН", "место поставки"]}`
> ```
> Few-shot улучшает точность на граничных случаях. Добавляйте в конец раздела `## Примеры` в промпт-файле.

> 💡 **Тест промпта без API-ключа:**
> ```bash
> python3 -c "
> from shared.llm import build_llm
> llm = build_llm()
> resp = llm.invoke(open('prompts/dzo_v1.md').read() + '\n\nЗаявка: тест')
> print(resp.content[:300])
> "
> ```
> Это всё равно требует API-ключ (LLM_API_KEY), но не требует запущенного сервера.
> Для полностью offline: используйте `FakeLLM` из `tests/fixtures/fake_llm.py`.
...
[🛡️ Урок 13 — Защита LLM: галлюцинации, контекст, дубли](lesson_13.md)

---

## ⚠️ Типичные ошибки при написании промптов

| Ошибка | Симптом | Решение |
|---|---|---|
| Нет секции запретов | Агент делает то, чего не должен | Добавить «НИКОГДА не делай: ...» в промпт |
| Смешать роль и алгоритм в одном блоке | Агент нарушает порядок шагов | Разделить на секции: Личность / Алгоритм / Запреты |
| Не версионировать промпт | Непонятно что изменилось после деградации | Всегда: `v1`, `v2` с датой и описанием изменений |
| Слишком абстрактные инструкции | «Будь точным» — LLM не понимает что значит точным | Конкретизировать: «Отвечай JSON с полями X, Y, Z» |


## ✅ Проверь себя

1. Назови 7 обязательных секций канонического промпта.
2. Почему секция «запреты» так же важна как «алгоритм»?
3. Как отличается промпт агента ДЗО от промпта агента Тендер?
4. Что произойдёт если убрать секцию «формат ответа»?
5. Открой `agent1_dzo_inspector/prompts/v1.py` — найди секцию «ЛИЧНОСТЬ».
