...
> ⬅️ [Урок 16 — Шаблоны промптов](lesson_16.md)

> 🎯 **Зачем этот урок?** Агент без тестов — как самолёт без чеклиста. Каждое изменение промпта или кода может сломать всю цепочку. После урока ты научишься запускать тесты, понимать пирамиду unit→integration→E2E и писать свой первый тест.

## Зачем тестировать агентов?

> 💡 **Аналогия:** Тест агента — как лётный чеклист пилота. Каждый пункт проверяется отдельно (unit), затем всё вместе (integration), и только потом — реальный полёт (E2E).
...
| Понятие | Смысл |
|---|---|
| Unit-тест | Быстро, без сети, без LLM — проверяет одну функцию |
| Integration-тест | Проверяет API и БД — нужен реальный сервер |
| E2E-тест | Полный сценарий с настоящей LLM — дорого, редко |
| `conftest.py` | Заглушки и фикстуры — подменяют LLM на mock |
| `pytest -m "not e2e"` | Запускает все тесты кроме E2E |
| `@pytest.mark.e2e` | Метка для тестов с реальной LLM |

---

## ⚠️ Типичные ошибки при тестировании агентов

| Ошибка | Почему плохо | Решение |
|---|---|---|
| Тест падает из-за случайности LLM | Нет детерминизма — CI нестабилен | Всегда мокировать LLM в unit/integration |
| E2E тест в CI с реальным ключом | Деньги тратятся при каждом push | Использовать `@pytest.mark.e2e` + ручной запуск |
| Тест проверяет слишком много | При сбое непонятно что сломалось | Один тест — одно утверждение |
| Нет `conftest.py` с фикстурами | Дублирование кода в каждом тесте | Вынести общие моки в `conftest.py` |
| Тест зависит от порядка запуска | Флакируют в разных сессиях | Изолировать состояние между тестами |
