...
> ⏱️ **25 минут** | 🟠 Средний
> ⬅️ [Урок 12 — Промпты](lesson_12_prompts.md)

> 🎯 **Зачем этот урок?** LLM может ошибаться — галлюцинировать, игнорировать правила, давать неточные ответы. Этот урок о том как этого не допустить в продакшене.
...
> 💡 **Аналогия:** Защита LLM — как охрана в банке: первый уровень — пропускной турникет (промпт), второй — охранник (валидация кода), третий — сейф (ограничения БД).
...
![Три уровня защиты LLM от галлюцинаций](images/lesson_13_defense_levels.jpg)
...
2. Что такое prompt injection — приведи пример атаки.
3. Почему нельзя доверять входным данным от пользователя без валидации?
4. Какой тип защиты самый важный: промпт, код или БД?
