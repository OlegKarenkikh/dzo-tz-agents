...
> ⏱️ **25 минут** | 🟠 Средний
> ⬅️ [Урок 7 — Агент как инструмент](lesson_07_agent_as_tool.md)

![MCP и A2A архитектура](images/lesson_08_mcp_a2a.jpg)

> 🎯 **Зачем этот урок?** Агент живёт не только в терминале — его можно вызвать из Claude Desktop, Cursor или другого агента в интернете. MCP и A2A — два стандарта, которые делают это возможным. После урока ты поймёшь разницу и когда использовать каждый.
...
> 💡 **Claude Desktop vs Cursor — когда что?**
>
> | Клиент | Лучше для | Особенность |
> |---|---|---|
> | Claude Desktop | Тестирование, демо | Стриминг шагов агента |
> | Cursor | Разработка, интеграция в IDE | Доступ прямо из редактора |
> | VS Code + MCP ext | Проекты на VS Code | Стандарт расширение |
...
![MCP vs A2A — два стандарта интеграции агентов](images/lesson_08_mcp_vs_a2a.jpg)
...
## ⚠️ Типичные ошибки при работе с MCP и A2A

| Ошибка | Причина | Решение |
|---|---|---|
| Агент не виден в Claude Desktop | Неверный порт или путь в конфиге | Проверить `claude_desktop_config.json` |
| SSE соединение обрывается | Таймаут по умолчанию слишком мал | Увеличить `keep_alive` в настройках |
| A2A вызов возвращает 401 | Не передан или неверный Bearer токен | Добавить `Authorization: Bearer {API_KEY}` |
| Агент не отвечает на peer-вызов | Другой агент не запущен | Проверить что все 4 сервиса активны |


## ✅ Проверь себя

1. Расшифруй MCP и A2A — что означают аббревиатуры?
2. В чём главное различие: кто с кем общается в MCP и A2A?
3. Какой стандарт используется в нашем проекте для peer-вызовов?
4. Зачем нужны два разных стандарта — нельзя ли обойтись одним?
