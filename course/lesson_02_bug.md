...
> 🎯 **Зачем этот урок?** Ошибки в коде — это норма, не катастрофа. После этого урока ты будешь читать Traceback как инструкцию: сразу видеть что сломалось и где.
...
> LOG_LEVEL=DEBUG
> ```
> Или запустите напрямую с флагом:
> ```bash
...
1. Что означает `Traceback (most recent call last)` — читать с начала или с конца?
2. Чем `KeyError` отличается от `TypeError`?
3. Что нужно сделать первым делом, когда видишь незнакомую ошибку?
4. Как запустить код в режиме отладки (`pdb`) и выйти из него?
5. Попробуй намеренно вызвать `NameError` — напиши `print(x)` без объявления `x`.
