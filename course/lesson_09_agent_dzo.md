...
## ⚠️ Типичные ошибки при работе с Агентом ДЗО

| Ошибка | Симптом | Решение |
|---|---|---|
| Передать письмо без вложений | Агент ищет ТЗ, не находит, зависает | Прикладывать все вложения в запросе |
| Не указать `request_id` | Дублирование обработки одного письма | Всегда генерировать уникальный `X-Request-ID` |
| Очень длинный промпт | Вытесняет контекст письма из окна | Промпт < 2000 токенов, данные — остальное |
| Пропустить статус `needs_clarification` | Агент просит данные, их не предоставляют | Обрабатывать все статусы ответа агента |
