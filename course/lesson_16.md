...
> ⏱️ **30 минут** | 🟠 Средний
> ⬅️ [Урок 15 — Агент Collector](lesson_15.md)

> 🎯 **Зачем этот урок?** Промпт — это код. Как и код, его нужно версионировать, тестировать и уметь откатывать. После урока ты будешь делать это профессионально.

![Шаблон правильного промпта — структура и версионирование](images/lesson_16_prompt_template.jpg)

---

## 🤔 Почему промпты — это код?

> 💡 **Аналогия:** Версионирование промптов — как git для кода: v1, v2, v3. Что-то сломалось — откатился на предыдущую версию одной строкой в config.py.
...
![7 обязательных секций промпта агента](images/lesson_16_7sections.jpg)
...
![Версионирование промптов — git история и откат](images/lesson_16_versioning.jpg)
...
➡️ **Следующий урок:** [Урок 17 — Тестирование агентов: unit, mock, e2e](lesson_17_testing.md)
...
1. Зачем нужно версионировать промпты — что было бы без этого?
2. Назови 7 обязательных секций промпта агента.
3. Что изменить в `config.py`, чтобы откатиться на предыдущую версию промпта?
4. Почему секция «запреты» важна так же, как секция «алгоритм»?
