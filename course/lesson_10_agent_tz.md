...
> ⏱️ **25 минут** | 🟠 Средний
> ⬅️ [Урок 9 — Агент ДЗО изнутри](lesson_09_agent_dzo.md)
...
## ⚠️ Типичные ошибки при работе с Агентом ТЗ

| Ошибка | Симптом | Решение |
|---|---|---|
| Передача всего письма вместо ТЗ | Агент анализирует переписку, а не документ | Извлечь только вложение или раздел ТЗ |
| Игнорировать статус `tz_not_found` | Система продолжает работу без ТЗ | Обрабатывать статус и возвращать пользователю запрос |
| Не проверять версию промпта агента | После обновления промпта поведение меняется | Фиксировать версию в конфиге и тестировать |
| Слишком короткий таймаут для больших ТЗ | `TimeoutError` на документах > 50 страниц | Увеличить таймаут для тяжёлых документов |
