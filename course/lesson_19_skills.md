...
> ⬅️ [Урок 18 — CI/CD](lesson_18_ci.md)
...
> 💡 **Аналогия:** Инструмент агента — как кнопка на пульте управления. Кнопка выполняет одно действие, а надпись (docstring) объясняет оператору (LLM) когда и зачем её нажимать.
...
1. Чем `@tool` отличается от обычной функции?
2. Зачем нужна `args_schema=...Input`?
3. Напиши хороший docstring для `check_budget(amount: float)`.
4. Почему короткий docstring — это баг?
5. Назови 5 типичных ошибок новичка при написании инструментов.


---

➡️ **[Следующий урок: Урок 20 — Страховые тендеры: лицензия ЦБ РФ и CBR Post-Check](lesson_20_insurance_cbr.md)**
