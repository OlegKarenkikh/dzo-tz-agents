...
> 💡 **Аналогия:** Инструмент агента — как кнопка на пульте управления. Кнопка (функция) выполняет одно действие, а надпись на ней (docstring) говорит оператору (LLM) — когда и зачем нажимать.
...
3. Напиши хороший docstring для инструмента `check_budget(amount: float)`.
4. Почему короткий docstring — это баг, а не удобство?
