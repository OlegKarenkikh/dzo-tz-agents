...
> ⏱️ **25 минут** | 🟠 Средний
> ⬅️ [Урок 14 — Peer-вызовы](lesson_14.md)

![Агент Collector — третий агент системы](images/lesson_15_agent3_collector.jpg)

> 🎯 **Зачем этот урок?** Агент Collector — третий специалист в системе. Он единственный, кто умеет ходить во внешние реестры (ФНС, РНП, ЕИС). После урока ты поймёшь зачем он нужен, что именно проверяет и как его вызвать из другого агента.
...
```python
# Из agent2_tz_inspector/tools.py
@tool
...
➡️ **Следующий урок:** [Урок 16 — Шаблоны промптов и версионирование](lesson_16.md)

---

## ⚠️ Типичные ошибки при работе с Агентом Collector

| Ошибка | Симптом | Решение |
|---|---|---|
| Передать невалидный ИНН | Collector делает запрос в реестр, получает 404 | Валидировать ИНН (10 или 12 цифр) до вызова |
| Ждать синхронного ответа от реестра | Реестры могут отвечать > 30с | Использовать асинхронные вызовы с `asyncio` |
| Кешировать результат навсегда | Устаревшие данные из ФНС или РНП | Устанавливать TTL кеша (не более 24 часов) |
| Не обрабатывать статус `service_unavailable` | Система падает если реестр недоступен | Graceful degradation — возвращать статус с пометкой |
