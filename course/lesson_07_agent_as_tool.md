...
> ⬅️ [Урок 6 — Что такое Tool](lesson_06_what_is_tool.md)

![Агент ДЗО вызывает Агент ТЗ](images/lesson_07_agent_as_tool.jpg)

> 🎯 **Зачем этот урок?** Агенты в проекте работают как команда: каждый делает своё. Здесь ты поймёшь как один агент запускает другого — и почему это делается через `@tool`, а не напрямую.
...
![Агент ТЗ вызывает агента ДЗО как инструмент](images/lesson_07_agent_as_tool.jpg)
...
## ⚠️ Типичные ошибки при вызове агентов друг из друга

| Ошибка | Почему возникает | Решение |
|---|---|---|
| Прямой `import` агента вместо `invoke_peer_agent` | Жёсткая зависимость, нет изоляции | Всегда использовать HTTP/A2A вызов |
| Передача полного контекста в peer-вызов | Агент получает лишние данные, путается | Передавать только релевантный фрагмент |
| Нет таймаута на peer-вызов | Зависание при недоступности агента | Устанавливать `timeout=30` в requests |
| Не обрабатываешь ошибку агента-получателя | Агент-вызывающий падает молча | Всегда проверять `status_code` ответа |


## ✅ Проверь себя

1. В чём разница между вызовом обычного `@tool` и вызовом агента как инструмента?
2. Почему агент ТЗ не выполняет логику ДЗО сам?
3. Что передаётся в `invoke_dzo_agent()` — только текст или структурированные данные?
4. Что произойдёт если агент ДЗО вернёт ошибку?
