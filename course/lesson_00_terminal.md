...
> 🎯 **Зачем этот урок?** После него ты сможешь открывать терминал, переходить между папками, запускать команды и не бояться чёрного экрана. Это фундамент — без терминала невозможно работать с Python и Git.
...
> Если вы не знаете что это — не волнуйтесь. Следующий урок полностью посвящён этой теме.
> Кратко: это изолированный Python-окружение для проекта — чтобы зависимости агентов не конфликтовали с другими Python-проектами.
...
## ⚠️ Типичные ошибки новичка в терминале

| Ошибка | Что происходит | Решение |
|---|---|---|
| `command not found` | Программа не установлена или не в PATH | Установить программу или проверить PATH |
| `Permission denied` | Нет прав на выполнение | Добавить `chmod +x файл` или использовать `sudo` |
| Закрыл терминал — потерял сервер | Процессы умирают вместе с сессией | Использовать `screen`, `tmux` или `nohup` |
| Выполнил команду не в той папке | Файлы создаются не там | Всегда проверяй `pwd` перед выполнением |
| `cd путь с пробелами` без кавычек | Ошибка парсинга пути | Использовать `cd "путь с пробелами"` |


## ✅ Проверь себя

1. Что делает команда `cd` и чем отличается `cd ..` от `cd ~`?
2. Как посмотреть список файлов в текущей папке (включая скрытые)?
3. Что такое PATH и зачем он нужен?
4. Как узнать, в какой папке ты находишься прямо сейчас?
5. Запусти в терминале `python --version` и `pip --version` — что ты видишь?
