...
## Email-раннер (`CollectorEmailRunner`)

Начиная с текущей версии, `runner.py` содержит класс `CollectorEmailRunner(BaseEmailRunner)` — IMAP-polling раннер, аналогичный `DzoEmailRunner` и `TzEmailRunner`.

| Хук | Реализация |
|-----|-----------|
| `agent_id` | `"collector"` |
| `imap_config` | `EMAIL_HOST`, `EMAIL_USER`, `EMAIL_PASSWORD`, `EMAIL_PORT`, `COLLECTOR_IMAP_FOLDER` |
| `create_agent()` | `create_collector_agent()` |
| `build_chat_input()` | Формирует строку из метаданных письма + тексты вложений |
| `parse_steps()` | Извлекает результат `collect_tender_documents` из `intermediate_steps` |
| `send_reply()` | Отправляет отчёт в формате `<pre>` через `shared/email_sender.py` |

Точка входа: `process_collector_emails()`.

> Шаги 1–3 (получение писем, дедупликация, обработка) и шаги финальные (ответ, метрики) реализованы в `shared/runner_base.py::BaseEmailRunner.process_emails()`.

---

## Интеграция с `shared/document_parser.py`

`tools.py` опционально импортирует `parse_anketa` из `shared/document_parser.py`:
...
Если модуль доступен и вложение содержит бинарные данные (`att["data"]`), анкета автоматически парсится для извлечения ИНН и других полей. Это повышает точность сверки с данными из сводной таблицы участников.

---

## Что агент НЕ делает

- **Не отправляет приглашения** — рассылка приглашений участникам ТО выполняется вручную.
- **Не оценивает качество** заполнения анкеты — проверяется только наличие и формальное соответствие полей.
- **Не принимает решений** о допуске участника — решение остаётся за сотрудником УЦЗ.
- **Не хранит оригиналы файлов** — агент анализирует метаданные и content_hint, но не сохраняет PDF/DOCX.
- **Не проверяет электронную подпись** — NDA и анкеты проверяются только на наличие, не на подлинность.
...
3. **Auto-parse анкеты:** поддерживается опциональный парсинг анкеты через `shared/document_parser.py`, если модуль доступен (graceful degradation при отсутствии зависимостей).
