...
# ── TTL-кеш для probe_max_input_tokens (24h, файл + память) ──────────────────
_TOKEN_LIMITS_TTL: int = 86_400
...
def _tlc_load() -> dict:
    try:
        if _TOKEN_LIMITS_CACHE_FILE.exists():
            return _json.loads(_TOKEN_LIMITS_CACHE_FILE.read_text())
    except Exception as exc:
        logger.warning("TTL-кеш: ошибка чтения %s: %s", _TOKEN_LIMITS_CACHE_FILE, exc)
    return {}


def _tlc_save(data: dict) -> None:
    try:
        _TOKEN_LIMITS_CACHE_FILE.write_text(_json.dumps(data))
    except Exception as exc:
        logger.warning("TTL-кеш: ошибка записи %s: %s", _TOKEN_LIMITS_CACHE_FILE, exc)


def _tlc_get(key: str) -> int | None:
    now = _time.time()
    with _llm_cache_lock:
        e = _TOKEN_LIMITS_MEM.get(key)
        if e and now - e["ts"] < _TOKEN_LIMITS_TTL:
            return e["value"]
        fd = _tlc_load()
        fe = fd.get(key)
        if fe and now - fe["ts"] < _TOKEN_LIMITS_TTL:
            _TOKEN_LIMITS_MEM[key] = fe
            return fe["value"]
    return None


def _tlc_set(key: str, value: int) -> None:
    entry = {"value": value, "ts": _time.time()}
    with _llm_cache_lock:
        _TOKEN_LIMITS_MEM[key] = entry
        fd = _tlc_load()
        fd[key] = entry
        _tlc_save(fd)
...
def probe_max_input_tokens(api_key: str, model_name: str) -> int:
    """RC-03 + TTL-кеш 24h: сначала проверяем файловый кеш, затем HTTP-проб."""
