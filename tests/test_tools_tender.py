...
class TestGenerateDocumentList:
    ...
    def test_all_document_types_accepted(self):
        ...
        for doc_type in valid_types:
            payload = json.dumps(
                {
                    ...
                    "documents": [
                        {
                            "name": f"Документ типа {doc_type}",
                            "type": doc_type,
                            "mandatory": True,
                            "section_reference": "п. 1",
                            "requirements": "",
                            "basis": "Прямое требование",
                        },
                    ],
            ...
            result = json.loads(generate_document_list.invoke(payload))
            assert result["documents"][0]["type"] == doc_type, (
                f"Тип {doc_type!r} должен приниматься"
            )
