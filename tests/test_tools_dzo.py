...
class TestGenerateValidationReport:
    ...
    def test_validation_error_on_wrong_types(self):
        with pytest.raises(ValidationError):
            generate_validation_report.invoke(
                {"decision": "ok", "checklist_attachments": "not-a-list"}
            )
        ...
        payload = json.dumps(
            {
                ...
                "recommended_suppliers": [
                    {"name": "<em>Evil Corp</em>", "inn": "<script>0</script>"}
                ],
                "additional_info": "ok",
